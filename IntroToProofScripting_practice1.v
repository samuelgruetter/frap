...
Proof.
  ...
  simplify;
  apply Inc2Inv'; simplify; try equality;
  unfold shared_from_private; simplify; equality.
Qed.
...
Ltac my_propositional :=
  repeat match goal with
         ...
         | |- _ /\ _ => constructor
         | H: _ \/ _ |- _ => cases H
         | H: ?P -> _, H': ?P |- _ => specialize H with (1 := H')
         | |- _ -> _ => intro
         end.
...
Proof.
  match goal with
    | [ |- _ ] => idtac "1"; intro; idtac "1b"
    | [ |- True ] => idtac "2"; constructor; idtac "2b"
  end.
Qed.
...
Ltac notHyp P :=
  match goal with
  | _: P |- _ => fail 1 P "is already known"
  | _ => match P with
         | ?P1 /\ ?P2 => first [notHyp P1 | notHyp P2 | fail 2 P1 "and" P2 "are already known"]
         | _ => idtac (* success *)
         end
  end.
...
Ltac extend pf :=
  ...
  match tt with
  | Prop => idtac
  | _ => fail 1 "type of" t "is not Prop"
  end;
  pose proof pf.
...
(*         | H: ?P |- ?P => exact H*)
         | H: True |- _ => clear H
         | H: forall (x: ?T), _, y: ?T |- _ => extend (H y); idtac "specialized" H "with" y
         | |- True => constructor
         | H: False |- _ => cases H
         | H: _ /\ _ |- _ => cases H
         | |- _ /\ _ => constructor
...
Section firstorder.
  ...
  Proof.
    repeat completer_step. assumption.
  Qed.
...
Ltac length ls :=
  match ls with
  | _ :: ?t => let r := length t in constr:(S r)
  | nil => constr:(0)
  end.
...
Ltac length ls k :=
  idtac ls;
  match ls with
  | _ :: ?t => length t ltac:(fun res => k constr:(S res))
  | nil => k constr:(0)
  end.

Goal False.
  length (1 :: 2 :: 3 :: nil) ltac:(fun res => pose res).
Abort.
*)

Ltac length ls :=
  let dummy := match constr:(42) with
               | _ => idtac ls; assert (ls = ls) by equality; idtac "yay"
               end in
  match ls with
  | _ :: ?t => let r := length t in constr:(S r)
  | nil => constr:(0)
...
Section test_inster.
  ...
  Proof.
    ...
    completer. auto. (* apply H7. apply H8. repeat split. all: assumption. *)
  Qed.
...
Theorem t55 : (forall x : nat, S x > x) -> 2 > 1.
Proof.
  intros.
  newEvar nat ltac:(fun e => specialize (H e)).
Abort.

(* Spec: instantiate initial [forall]s of [H] with new evars. *)
Ltac insterU H :=
  repeat match type of H with
         | forall (x: ?T), _ => newEvar T ltac:(fun e => specialize (H e))
         end.
...
Ltac insterU tac H ::=
  repeat match type of H with
         | forall (x: ?T), _ =>
           ...
           | Prop =>
             let Htemp := fresh in
             assert T as Htemp by tac;
             specialize (H Htemp);
             clear Htemp
           | _ => newEvar T ltac:(fun e => specialize (H e))
           end
         end.

Ltac insterKeep tac H ::=
  let Hfresh := fresh H "i" in
  pose proof H as Hfresh;
  insterU tac Hfresh.
